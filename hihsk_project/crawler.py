...
MEDIA_BASE = "https://media.hihsk.com"

# Quy luat URL media (xac dinh qua DevTools + test thuc te):
#   Image:   folder LUON = 1   -> https://media.hihsk.com/vocabulary/images/1/{image_10}
#   Mp3 nam: folder = part     -> https://media.hihsk.com/vocabulary/male/{part}/{mp3}
#   Mp3 nu:  folder = part     -> https://media.hihsk.com/vocabulary/female/{part}/{mp3_female}
# (truong "part" cua tung tu trong JSON quyet dinh folder audio)

def build_image_url(filename):
    if not filename: return None
    if filename.startswith("http"): return filename
    return f"{MEDIA_BASE}/vocabulary/images/1/{filename}"

def build_mp3_url(filename, part, gender="male"):
    if not filename or part is None: return None
    if filename.startswith("http"): return filename
    return f"{MEDIA_BASE}/vocabulary/{gender}/{part}/{filename}"
...
def parse_vocabulary(file_id, data):
    ...
    for v in data.get("vocals", []):
        ...
        saved += db.save_vocab({
            ...
            "image_url":        build_image_url(v.get("image_10")),
            "audio_mp3":        build_mp3_url(v.get("mp3"),        v.get("part"), "male"),
            "audio_mp3_female": build_mp3_url(v.get("mp3_female"), v.get("part"), "female"),
        ...
        for s in (v.get("sentences") or []):
            db.save_sentence({
                ...
                "audio_mp3": build_mp3_url(s.get("mp3"), v.get("part"), "male"),
            })
