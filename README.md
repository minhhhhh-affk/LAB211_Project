...
Chuỗi base64 dài đó chính là **AES Secret Key**.

### Bước 4 — Xác minh bằng Console

F12 → **Console** → gõ `allow pasting` → Enter → dán đoạn sau:
...
performance.getEntriesByType('resource')
  ...
  .forEach(r => {
    fetch(r.name)
      .then(res => res.text())
      .then(t => {
        if(t.includes('secretKeyBase64')) {
          const m = t.match(/secretKeyBase64[^"]*"([A-Za-z0-9+/=]{40,})"/);
          if(m) console.log('SECRET KEY:', m[1]);
        }
      });
  });
```
...
Đây là lỗ hổng **hardcoded secret** — một trong những lỗi bảo mật phổ biến nhất trong phát triển phần mềm:
